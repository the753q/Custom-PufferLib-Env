...
void generate_brick_positions(MyEnv* env) {
...
void compute_observations(MyEnv* env) {
...
static inline void calc_brick_collision(MyEnv* env, int idx, 
...
static inline int column_index(MyEnv* env, float x) {
    return (int)(x / env->brick_width);
}
static inline int row_index(MyEnv* env, float y) {
    return (int)((y - Y_OFFSET) / env->brick_height);
}

void calc_all_brick_collisions(MyEnv* env, CollisionInfo* collision_info) {
...
bool calc_paddle_ball_collisions(MyEnv* env, CollisionInfo* collision_info) {
...
void calc_all_wall_collisions(MyEnv* env, CollisionInfo* collision_info) {
    if (env->ball_vx < 0) {
...
void check_wall_bounds(MyEnv* env) {
...
void destroy_brick(MyEnv* env, int brick_idx) {
...
bool handle_collisions(MyEnv* env) {
    CollisionInfo collision_info = {
        .t = 2.0f,
        .overlap = -1.0f,
        .x = 0.0f,
...
void step_frame(MyEnv* env, float action) {
    float act = 0.0;
...
Client* make_client(MyEnv* env) {
    ...
    InitWindow(env->width, env->height, "PufferLib MyEnv");
